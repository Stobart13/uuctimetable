...
if (is_file($csv_file)) {
    ...
    while ($row = fgetcsv($input, 1024, ',')) {
        ...
        $sql = 'INSERT INTO module(moduleid,modulecode,modulename,lecturername,modulelocation) VALUES(:moduleid,:modulecode,:modulename,:lecturername,:modulelocation)';
        $query = $db->prepare($sql);
        $query->bindParam(':moduleid', $row[0], PDO::PARAM_INT);
        $query->bindParam(':modulecode',$row[1], PDO::PARAM_STR);
        $query->bindParam(':modulename', $row[2], PDO::PARAM_STR);
        $query->bindParam(':lecturername', $row[3], PDO::PARAM_STR);
        $query->bindParam(':modulelocation', $row[4], PDO::PARAM_STR);
        $query->execute();
    }
}
?>
...
<head>
        <title>Ulster University Timetable - Add Module by CSV</title>
...
<body>
<h2>Add Modules via CSV</h2>
        <div class="form_upload">
            <h2>Upload CSV file</h2>
            <form action="addModuleCSV.php" method="post" enctype="multipart/form-data">
